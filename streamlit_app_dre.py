...
st.set_page_config(page_title="DRE Dinâmico — BD + Filtro por Mês", layout="wide")
...
COLUMN_MAP = {
    ...
    "fim_mes": ["FIM MES", "FIM MÊS", "FIM_DO_MES", "FIM DO MES", "DATA FIM MES", "DATA FIM MÊS", "FIM-MES"],
...
def normalize_bd(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fim_col = find_col(df, "fim_mes")
    if fim_col and fim_col != "FIM MÊS":
        df = df.rename(columns={fim_col: "FIM MÊS"})
    ...
    if "FIM MÊS" in df.columns:
        df["FIM MÊS"] = pd.to_datetime(df["FIM MÊS"], errors="coerce").dt.date
        # Se MÊS/ANO não existir, derive de FIM MÊS
        if "MÊS/ANO" not in df.columns:
            df["MÊS/ANO"] = pd.to_datetime(df["FIM MÊS"]).dt.strftime("%Y-%m")
...
def to_numeric_cols(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.copy()
    protect = {"EMPRESA", "CLIENTE", "MÊS/ANO", "FIM MÊS"}
    for c in df.columns:
        if c in protect:
            continue
...
def build_filter_options(df_bd: pd.DataFrame):
    ...
    # Período via FIM MÊS
    min_d = max_d = None
    if "FIM MÊS" in df_bd.columns:
        series = df_bd["FIM MÊS"].dropna()
        if not series.empty:
            min_d = series.min()
            max_d = series.max()
    return emp_opts, cli_opts, mes_opts, (min_d, max_d), ("FIM MÊS" in df_bd.columns)

def apply_filters(df: pd.DataFrame, emp_sel, cli_sel, mes_sel, periodo, modo_periodo):
    if df.empty: return df
    out = df.copy()
    ...
    # Período
    if modo_periodo == "FIM MÊS (intervalo)" and "FIM MÊS" in out.columns and periodo and all(periodo):
        ini, fim = periodo
        out = out[(out["FIM MÊS"] >= ini) & (out["FIM MÊS"] <= fim)]
    elif modo_periodo == "MÊS/ANO (lista)" and "MÊS/ANO" in out.columns and mes_sel:
...
def kpi_row(df: pd.DataFrame):
    ...
    rec_col = None
    res_col = None
    for cand in COLUMN_MAP["receita_liquida"]:
        rec_col = next((c for c in df.columns if cand in c.upper()), rec_col)
    for cand in COLUMN_MAP["resultado"]:
        res_col = next((c for c in df.columns if cand in c.upper()), res_col)
...
st.title("📊 DRE Dinâmico — Base BD com Filtro por Mês")
...
emp_opts, cli_opts, mes_opts, periodo_limits, has_fim_mes = build_filter_options(bd)
emp_sel = st.sidebar.multiselect("Empresa", emp_opts)
cli_sel = st.sidebar.multiselect("Cliente", cli_opts)

# Se tiver FIM MÊS e MÊS/ANO, deixa escolher o modo. Caso contrário, escolhe o único disponível.
if has_fim_mes and mes_opts:
    modo_periodo = st.sidebar.radio("Critério de período", ["FIM MÊS (intervalo)", "MÊS/ANO (lista)"], horizontal=False)
elif has_fim_mes:
    modo_periodo = "FIM MÊS (intervalo)"
else:
    modo_periodo = "MÊS/ANO (lista)"

mes_sel = []
periodo = None
if modo_periodo == "FIM MÊS (intervalo)":
    if periodo_limits[0] and periodo_limits[1]:
        dmin, dmax = periodo_limits
        periodo = st.sidebar.date_input("Período (FIM MÊS)", value=(dmin, dmax))
else:
    mes_sel = st.sidebar.multiselect("Mês/Ano (YYYY-MM)", mes_opts, default=mes_opts[-1:] if mes_opts else [])

bd_filtrada = apply_filters(
    bd,
    emp_sel=emp_sel,
    cli_sel=cli_sel,
    mes_sel=mes_sel,
...
with st.expander("Diagnóstico"):
    ...
    if "FIM MÊS" in bd.columns:
        st.write("Período detectado (FIM MÊS):", bd["FIM MÊS"].min(), "→", bd["FIM MÊS"].max())
