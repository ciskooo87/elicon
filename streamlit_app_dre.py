...
def load_data(path: str, sheet_name: str = "bd") -> pd.DataFrame:
    ...
    # Novo: padroniza coluna de período "MÊS REF" (EOM)
    # Se existir, garante datetime
    if "MÊS REF" in df.columns:
        df["MÊS REF"] = pd.to_datetime(df["MÊS REF"], errors="coerce")
        # Cria label em pt-BR estilo "setembro/25"
...
if "MÊS REF" in df.columns:
    ...
    # Seleciona último período como default
    ...
    # Converte label selecionado de volta para data (alinha por índice)
...
st.caption("Leitura da base 'BD.xlsx' (aba 'bd'), filtros por EMPRESA e período com 'MÊS REF' (fim do mês).")
