...
st.set_page_config(page_title="DRE Dinâmico — BD + Período (Fim Mês)", layout="wide")
...
def build_filter_options(df_bd: pd.DataFrame):
    ...
    if "FIM MÊS" in df_bd.columns:
        ...
        if not series.empty:
            min_d = series.min()
            max_d = series.max()
    return emp_opts, cli_opts, mes_opts, (min_d, max_d)

def apply_filters(df: pd.DataFrame, emp_sel, cli_sel, mes_sel, periodo):
    if df.empty: return df
    out = df.copy()
    ...
    # Período tem prioridade quando existir FIM MÊS
    if "FIM MÊS" in out.columns and periodo and all(periodo):
        ini, fim = periodo
        out = out[(out["FIM MÊS"] >= ini) & (out["FIM MÊS"] <= fim)]
    elif "MÊS/ANO" in out.columns and mes_sel:
...
st.title("📊 DRE Dinâmico — Base BD com Filtro de Período (FIM MÊS)")
...
emp_opts, cli_opts, mes_opts, periodo_limits = build_filter_options(bd)
...
if periodo_limits[0] and periodo_limits[1]:
    ...
    # default: todo o range disponível
    periodo = st.sidebar.date_input("Período (FIM MÊS)", value=(dmin, dmax))
else:
    # fallback para MÊS/ANO se FIM MÊS não existir
...
bd_filtrada = apply_filters(
    ...
    emp_sel=emp_sel,
    cli_sel=cli_sel,
    mes_sel=[] if periodo else (mes_sel if 'mes_sel' in locals() else []),
