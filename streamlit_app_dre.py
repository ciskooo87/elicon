...
if aba == "Orçado x Realizado":
    ...
    # Resolve EMPRESA and período columns *inside budget df*
    bud_empresa_col = "EMPRESA" if "EMPRESA" in dfb.columns else resolve_col_ci(dfb, ["empresa", "cliente", "contrato", "razao social"], fallback_first=True)
    if "MÊS REF" in dfb.columns:
        bud_periodo_kind = "date"
        bud_periodo_col = "MÊS REF"
    else:
        # try common textual period columns
        bud_periodo_col = resolve_col_ci(dfb, ["times", "periodo", "mês", "mes", "competencia"], fallback_first=False)
        bud_periodo_kind = "text"

    # Filter both bases by same client and MÊS REF / legacy fallback
    if "MÊS REF" in df.columns and bud_periodo_kind == "date":
        dff_real = df[(df[col_empresa].astype(str) == str(cliente_sel)) & (df["MÊS REF"] == periodo_sel_dt)].copy()
        dff_bud  = dfb[(dfb[bud_empresa_col].astype(str) == str(cliente_sel)) & (dfb["MÊS REF"] == periodo_sel_dt)].copy()
    else:
        real_mask = (df[col_empresa].astype(str) == str(cliente_sel)) & (df[col_periodo].astype(str) == str(label_sel))
        dff_real = df[real_mask].copy()
        if bud_periodo_kind == "text" and bud_periodo_col is not None:
            bud_mask = (dfb[bud_empresa_col].astype(str) == str(cliente_sel)) & (dfb[bud_periodo_col].astype(str) == str(label_sel))
            dff_bud = dfb[bud_mask].copy()
        else:
            # Fallback: if nothing matches, empty frame to avoid KeyError
            dff_bud = dfb.iloc[0:0].copy()
