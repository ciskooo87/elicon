...
def to_numeric_br(s):
    """
    Converte série/coluna com valores em formato BR (ponto milhar, vírgula decimal)
    para float. Aceita números já float/int.
    """
...
if aba in ["DRE por Cliente", "DRE Consolidado"]:
    ...
    # Resolve coluna EMPRESA no orçamento (pode ter outro nome)
    bud_empresa_col = "EMPRESA" if "EMPRESA" in dfb.columns else resolve_col_ci(
        dfb, ["empresa","cliente","contrato","razao social","razão social"]
    )

    # Filtro: realizado por cliente + mês; orçado só por cliente (posição fixa)
    if "MÊS REF" in df.columns:
    ...
    dff_bud = dfb[dfb[bud_empresa_col].astype(str) == str(cliente_sel)].copy()

    # --------- Mapeamento de colunas do orçamento (auto + UI se precisar)
    ...
    with st.expander("Mapeamento do Orçado (caso as colunas tenham nomes diferentes)", expanded=False):
        st.caption("Se algum campo ficar em branco, consideramos zero para aquela linha.")
        cols = list(dfb.columns)
        def selbox(current, label):
            return st.selectbox(label, options=["<não usar>"] + cols, index=(cols.index(current)+1 if current in cols else 0))
        bud_col_fat = selbox(bud_col_fat if bud_col_fat else "", "Faturamento (orçado)")
        bud_col_ded = selbox(bud_col_ded if bud_col_ded else "", "Deduções (orçado)")
        bud_col_sal = selbox(bud_col_sal if bud_col_sal else "", "Salário (orçado)")
        bud_col_vt  = selbox(bud_col_vt if bud_col_vt else "", "Vale Transporte (orçado)")
        bud_col_va  = selbox(bud_col_va if bud_col_va else "", "Vale Alimentação (orçado)")
        bud_col_vr  = selbox(bud_col_vr if bud_col_vr else "", "Vale Refeição (orçado)")
        bud_col_ass = selbox(bud_col_ass if bud_col_ass else "", "Assiduidade (orçado)")
        bud_col_enc = selbox(bud_col_enc if bud_col_enc else "", "Total Encargos (orçado)")
        bud_col_ft  = selbox(bud_col_ft if bud_col_ft else "", "FT (orçado)")
        bud_col_frl = selbox(bud_col_frl if bud_col_frl else "", "Freelance (orçado)")
        bud_col_rat = selbox(bud_col_rat if bud_col_rat else "", "Rateio MP (orçado)")
        bud_col_mco = selbox(bud_col_mco if bud_col_mco else "", "Material de Consumo (orçado)")

        def nz(colname):
            return None if (colname is None or colname == "<não usar>") else colname
        bud_col_fat, bud_col_ded, bud_col_sal, bud_col_vt, bud_col_va, bud_col_vr = map(nz, [bud_col_fat, bud_col_ded, bud_col_sal, bud_col_vt, bud_col_va, bud_col_vr])
        bud_col_ass, bud_col_enc, bud_col_ft, bud_col_frl, bud_col_rat, bud_col_mco = map(nz, [bud_col_ass, bud_col_enc, bud_col_ft, bud_col_frl, bud_col_rat, bud_col_mco])

    bud_cost_cols = [c for c in [bud_col_sal, bud_col_vt, bud_col_va, bud_col_vr, bud_col_ass, bud_col_enc, bud_col_ft, bud_col_frl, bud_col_rat, bud_col_mco] if c]

    # --------- Coerção numérica (orçado pode estar em string BR)
    numeric_cols = [c for c in [bud_col_fat, bud_col_ded] + bud_cost_cols if c]
    for c in numeric_cols:
        dff_bud[c] = to_numeric_br(dff_bud[c])

    # Compute blocks
    def compute_block_local(df_block, col_fat, col_ded, cost_cols):
        fat = df_block[col_fat].fillna(0).sum() if col_fat else 0
        ded = df_block[col_ded].fillna(0).sum() if col_ded else 0
        fat_liq = fat - ded
        csp = df_block[cost_cols].fillna(0).sum().sum() if cost_cols else 0
    ...
    bud_blk  = compute_block_local(dff_bud, bud_col_fat, bud_col_ded, bud_cost_cols)
    ...
    st.divider()
    # --------- DRE completo lado a lado (Real vs Orçado)
    st.markdown("## DRE Completo – Realizado vs Orçado (mês selecionado)")
    colL, colR = st.columns(2)
    with colL:
        st.markdown("### DRE – Realizado")
        block_dre("", dff_real)
    with colR:
        st.markdown("### DRE – Orçado")
        # Render DRE-like block for Orçado usando as colunas mapeadas
        fat = dff_bud[bud_col_fat].fillna(0).sum() if bud_col_fat else 0
        ded = dff_bud[bud_col_ded].fillna(0).sum() if bud_col_ded else 0
        fatliq = fat - ded
        csp_sum = dff_bud[bud_cost_cols].fillna(0).sum().sum() if bud_cost_cols else 0
        mc = fatliq - csp_sum
        def linha_orc(label, valor, base):
            c1o, c2o, c3o = st.columns([2.8, 1.2, 0.8])
            with c1o: st.markdown(label)
            with c2o: st.markdown(f"<div style='text-align:right'>{money(valor)}</div>", unsafe_allow_html=True)
            with c3o: st.markdown(f"<div style='text-align:right'>{perc(pct(valor, base))}</div>", unsafe_allow_html=True)
        st.divider()
        st.markdown("#### ORÇADO | AV%")
        linha_orc("(+) FATURAMENTO BRUTO", fat, fat if fat else 1)
        linha_orc("(–) DEDUÇÕES LEGAIS", ded, fat if fat else 1)
        linha_orc("(=) FATURAMENTO LÍQUIDO", fatliq, fat if fat else 1)
        st.divider()
        linha_orc("(–) CSP", csp_sum, fat if fat else 1)
        # quebra por custo se houver
        if bud_cost_cols:
            for cname in bud_cost_cols:
                linha_orc(f"(–) {cname}", dff_bud[cname].fillna(0).sum(), fat if fat else 1)
        st.divider()
        linha_orc("(=) MARGEM DE CONTRIBUIÇÃO", mc, fat if fat else 1)
