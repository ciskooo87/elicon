...
if aba in ["DRE por Cliente", "DRE Consolidado"]:
    ...
    if not agg_dict:
        ...
        # Top 10 melhores e piores Margens – modo dinâmico
        if mc_mode == "Percentual (%)":
            ...
            st.markdown("### Top 10 – Maiores Margens de Contribuição (R$) (mês selecionado)")
            top_mc_best = by_emp.sort_values("MARGEM_CONTRIB", ascending=False).head(10)
            st.bar_chart(top_mc_best["MARGEM_CONTRIB"])
            st.dataframe(top_mc_best[["MARGEM_CONTRIB", "FATURAMENTO_BRUTO"]])

            st.markdown("### Top 10 – Menores Margens de Contribuição (R$) (mês selecionado)")
            top_mc_worst = by_emp.sort_values("MARGEM_CONTRIB", ascending=True).head(10)
            st.bar_chart(top_mc_worst["MARGEM_CONTRIB"])
            st.dataframe(top_mc_worst[["MARGEM_CONTRIB", "FATURAMENTO_BRUTO"]])
