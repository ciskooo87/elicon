...
def resolve_col_ci(df: pd.DataFrame, targets: list[str], fallback_first: bool = True):
    """
    Resolve a column name by case-insensitive match against a list of target aliases.
    If not found and fallback_first=True, returns the first column.
    """
...
if aba == "Orçado x Realizado":
    ...
    # Resolve column 'EMPRESA' (budget may name it differently)
    bud_empresa_col = "EMPRESA" if "EMPRESA" in dfb.columns else resolve_col_ci(dfb, ["empresa","cliente","contrato","razao social","razão social"], fallback_first=True)

    # Filter realizado by month/client; budget ONLY by client (no period in budget)
    if "MÊS REF" in df.columns:
